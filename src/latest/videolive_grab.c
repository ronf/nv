...
static void VideoLive_GrabColor(uint8 *y_data, uint8 *uv_data)
{
    int x, y;
...
static int VideoLive_GrabFrame(uint8 *y_data, uint8 *uv_data)
{
    if (xmit_color)
...
#ifdef VIDEOLIVE_DIRECT
static void VideoLive_GrabColorDirect(uint8 *y_data, uint8 *uv_data,
				      uint32 *data)
{
    int x, y;
    uint8 *yp=y_data, *ry=rgb2y;
...
static int VideoLive_GrabFrameDirect(uint8 *y_data, uint8 *uv_data)
{
    if (xmit_color)
...
grabproc_t *VideoLive_Start(int min_framespacing, int config, reconfigproc_t *r,
			    void *e)
{
    ...
    /* XXX: Need to redo this to handle restarts better, rather than shutting
       everything down and restarting it again */
    if (reconfig != NULL) VideoLive_Stop();
