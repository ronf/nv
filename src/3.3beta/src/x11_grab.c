...
static int X11Grab_MSBWhite1(uint8 *y_data, int8 *uv_data)
{
    ...
    if (xmit_color) memset(uv_data, 0, width*height);
...
int X11Grab_Probe(void)
{
    ...
    init = X11Grab_Initialize(rw, width, height);
    ...
    return init? VID_GREYSCALE|VID_COLOR|VID_SMALL|VID_MEDIUM|VID_LARGE : 0;
}
